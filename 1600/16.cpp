...
struct Interactor{
    ...
    vll Query_Interactor(ll s,ll e){
        vll res;
        for(ll i=s-1;i<e;i++) res.pb(a[i]);
        srt(res);
        return res;
    }
};
/*--------------------------------------------------------------------------------------------------------------------------------------------*/
vll Query(ll s,ll e,Interactor &it){
    cout<<"? "<<s<<" "<<e<<nl;
    vll a;
    if(it.testing1){
        a=it.Query_Interactor(s,e);
    }
    else{
        ll x;
...
bool possible(vll &a,ll s,ll e){
    ll cnt=0;
    for(ll i=0;i<a.size();i++) if(a[i]>=s && a[i]<=e) cnt++;
    return (cnt%2==1);
}
int main()
{
    ...
    while(t--){
        ...
        for(ll i=0;i<15 && s<=e;i++){
            if(s==e){
                ans=s;
                break;
            }
            mid=s+(e-s)/2;
            vll qarr=Query(s+1,mid+1,it);
            if(possible(qarr,s+1,mid+1)) e=mid;
            else s=mid+1;
        }
        cout<<"! "<<ans+1<<nl;
    }    
