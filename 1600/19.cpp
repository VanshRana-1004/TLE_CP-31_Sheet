...
bool possible(ll x,vll &a,vll &b){
    ll rich=x-1,poor=0,cnt=0;
    for(ll i=0;i<a.size();i++) if(a[i]>=rich && b[i]>=poor) rich--,poor++,cnt++;
    return cnt>=x;
}
int main()
{
    ...
    while(t--){
        ...
        ll s=1,e=n,mid,ans=1;
        while(s<=e){
            mid=s+(e-s)/2;
            if(possible(mid,a,b)) ans=mid,s=mid+1;
            else e=mid-1;
        }
        cout<<ans<<nl;
    }    
