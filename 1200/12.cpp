...
int main()
{
    ...
    while(t--){
        ...
        ll i=n-1,j=n-1;
        while(i>=0 && j>=0){
            ll val1=yx[i].first,ind1=yx[i].second;
            ll val2=xy[j].first,ind2=xy[j].second;
            if(ind1==ind2 || mp.count(ind1)) i--;
            else if(mp.count(ind2) || val1<val2) j--;
            else if(val1>=val2) ans++,mp[ind1]=mp[ind2]=true,i--,j--;
        }       
        cout<<ans<<nl;
    }    
}
