...
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n,ans=0,start=1e9,c=0;
        cin>>n;
        vll arr(n);
        read(arr,n);
        map<ll,ll> cnt;
        for(ll i=0;i<n;i++) cnt[arr[i]]++,start=min(start,arr[i]);
        for(auto x : cnt){
            if((start==x.first || start+1==x.first) && c<x.second) ans+=x.second-c;
            else if(!(start==x.first || start+1==x.first)) ans+=x.second;
            start=x.first;
            c=x.second;
        }
        cout<<ans<<nl;
