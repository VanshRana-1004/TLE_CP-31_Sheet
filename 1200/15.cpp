...
int main()
{
    ...
    while(t--){
        ...
        vll a(n);
        read(a,n);
        for(ll i=0;i<n;i++) if(a[i]==1) cur++;
        if(cur<s) cout<<-1<<nl;
        else if(cur==s) cout<<0<<nl;
        else{
            vll ind;
            for(ll i=0;i<n;i++) if(a[i]==1) ind.pb(i);
            ll del=ind.size()-s;
            ll s=del-1,e=ind.size(),ans;
            while(s>=0){
                if(e==ind.size()) ans=ind[s]+1;
                else ans=min(ans,ind[s]+1+n-ind[e]);
                s--;
                e--;
            }
            ans=min(ans,n-ind[ind.size()-del]);
            cout<<ans<<nl;
        } 
    }    
}
