...
int main()
{
    ...
    while(t--){
        ...
        for(ll i=0;i<n;i++){
            if(arr[i]%10==0){
                ll c=0,d=0;
                while(arr[i]>0 && arr[i]%10==0) arr[i]/=10,c++,d++;
                while(arr[i]>0) arr[i]/=10,d++;
                store.push_back({c,d});
            }
            else while(arr[i]>0) arr[i]/=10,ans++;
        }   
        srtr(store);
        ll i=0;
        while(i<store.size()){
            if(i%2==0) ans+=store[i].second-store[i].first;
            else ans+=store[i].second;
            i++;
        }
        if(ans>=m+1) cout<<"Sasha"<<nl;
        else cout<<"Anna"<<nl;
    }    
}
